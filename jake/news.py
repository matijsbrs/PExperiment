...
if __name__ == "__main__":
    ...
    for url in urls:
      ...
      try:
         chunks = chunker(text, 50) # Use the chunk_text function from web_utils
      except LookupError:
         print("Error: The punkt tokenizer is not downloaded. downloading now...")
         import nltk
         nltk.download('punkt')
         print("Retrying...")
         chunks = chunker(text)

      embeddings = model.encode(chunks[:10])
    ...
    while True:
      ...
      payload = {
        "model": "mistral:7b", # mistral-openorca
        "prompt": question, 
        "system": systemPrompt,
        "stream": False,
        "context": context
